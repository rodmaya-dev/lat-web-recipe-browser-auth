...
function printNotRun(label) {
  console.log(grey(`⚪ ${label} (no ejecutado)`));
  notRun++;
}
...
export function runGates(client) {
  ...
  if (built.ok) {
    console.log("✅ La app compila y se ejecuta sin errores");
    pass++;
  } else {
    console.log("❌ La app compila y se ejecuta sin errores — la compilación de Vite falló\n");
  ...
  if (compiled.ok) {
    console.log("✅ El proyecto compila sin errores de tipos\n");
    pass++;
  } else {
    console.log(
      "❌ El proyecto compila sin errores de tipos — corrige los errores de TypeScript\n",
    );
...
export function summary(encodedCode) {
  let line = `\nCorrectas: ${pass}, fallidas: ${fail}`;
  if (notRun > 0) line += grey(`, sin ejecutar: ${notRun}`);
  console.log(line);
  if (fail === 0 && notRun === 0) {
...
export function checkBehavior(client, testFile) {
  ...
  return {
    ...
    tests,
    message: !ok
      ? `\n- Ejecuta \`npx vitest run ${testFile}\` en client/ para ver la salida completa de vitest. Para guardar los resultados en un archivo,\n- ejecuta \`NO_COLOR=1 npx vitest run ${testFile} > results.txt\`
`
      : undefined,
  };
}
