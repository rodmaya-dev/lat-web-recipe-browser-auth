...
describe("Lección 04 — AuthContext", () => {
  beforeEach(() => {
    localStorage.clear();
    stubFetch();
  });

  it("empieza sin sesión iniciada", async () => {
    renderProbe();
    expect(await screen.findByText("signed out")).toBeInTheDocument();
  });

  it("login guarda el usuario actual y el token", async () => {
  ...
  it("logout limpia el usuario y elimina el token", async () => {
    const user = userEvent.setup();
    renderProbe();
