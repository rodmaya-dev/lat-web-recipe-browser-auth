...
describe("Lección 07 — protección de rutas", () => {
  beforeEach(() => {
    localStorage.clear();
    stubFetch();
  });

  it("redirige a los visitantes no autenticados de una ruta protegida a /login", async () => {
    renderAt("/");
  ...
  it("muestra las rutas públicas a los visitantes no autenticados", async () => {
    renderAt("/login");

    expect(await screen.findByText("login page stub")).toBeInTheDocument();
  });

  it("redirige a los usuarios autenticados fuera de las rutas públicas y muestra el contenido protegido", async () => {
